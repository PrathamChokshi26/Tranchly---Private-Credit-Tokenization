...
export default function YieldHistory() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Repayment Distributions</h1>
        <p className="text-gray-500 text-sm">Track distributions received from borrower repayments</p>
      </div>

      <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-xs text-amber-800" data-testid="yield-disclaimer-banner">
        <strong>Important:</strong> Returns are not guaranteed. Principal is at risk. Past performance does not predict future results.
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border p-5">
          <div className="flex items-center gap-2 mb-2"><DollarSign size={18} className="text-emerald-500" /> <span className="text-sm text-gray-500">Total Distributions</span></div>
      ...
      {cumulativeData.length > 0 ? (
        <>
          <div className="bg-white rounded-xl border p-5">
            <h3 className="font-semibold text-gray-900 mb-4">Cumulative Distributions</h3>
            <ResponsiveContainer width="100%" height={280}>
              <AreaChart data={cumulativeData}>
                ...
                <Tooltip />
                <Area type="monotone" dataKey="cumulative" stroke="#10b981" fill="url(#yg)" name="Total Distributions ($)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>

          <div className="bg-white rounded-xl border p-5">
            <h3 className="font-semibold text-gray-900 mb-4">Daily Distributions</h3>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={dailyYields}>
                ...
                <YAxis tick={{ fontSize: 11 }} />
                <Tooltip />
                <Bar dataKey="amount" fill="#8b5cf6" radius={[4, 4, 0, 0]} name="Distribution ($)" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </>
        ...
        <div className="bg-white rounded-xl border p-12 text-center">
          ...
          <p className="text-gray-500">No distributions received yet. Invest in loans to start receiving repayment distributions.</p>
        </div>
      )}
      ...
      {yields.length > 0 && (
        <div className="bg-white rounded-xl border">
          <div className="px-5 py-4 border-b">
            <h3 className="font-semibold text-gray-900">All Distributions</h3>
          </div>
            ...
            {yields.map(y => (
              <div key={y.id} className="flex items-center gap-4 px-5 py-3 text-sm">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-400">{y.token_count} loan shares • {new Date(y.created_at).toLocaleString()}</p>
                </div>
