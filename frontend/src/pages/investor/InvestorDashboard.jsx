    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm">Track your investments and yield</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={TrendingUp} label="Yield Earned" value={`$${(portfolio?.total_yield_earned || 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}`} color="emerald" />
        ...
        <div className="bg-white rounded-xl border p-5">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={cumulativeData}>
              ...
              <Tooltip />
              <Area type="monotone" dataKey="cumulative" stroke="#8b5cf6" fill="url(#yieldGrad)" name="Total Yield ($)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      )}
            ...
            {portfolio.investments.map(inv => (
              <Link key={inv.id} to={`/investor/marketplace/${inv.loan_id}`} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-500">{inv.token_count} tokens • {inv.loan?.industry}</p>
                </div>
                <GradeBadge grade={inv.loan?.grade} size="sm" />
                <div className="text-right">
                  <p className="font-medium">${inv.amount_invested?.toLocaleString()}</p>
                  <p className="text-xs text-emerald-600">+${(inv.yield_earned || 0).toFixed(2)} yield</p>
                </div>
